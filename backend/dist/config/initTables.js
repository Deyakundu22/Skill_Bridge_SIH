...
export const initTables = async () => {
    ...
    `);
        ...
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN issue_date DATE NULL`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN credential_id VARCHAR(255) NULL`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN file_name VARCHAR(255) NULL`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN file_type VARCHAR(100) NULL`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN file_size INT NULL`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN verification_status VARCHAR(50) DEFAULT 'pending'`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN verified_by INT NULL`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN verified_at TIMESTAMP NULL`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE student_certifications ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP`);
        }
        catch (_e) { }
    ...
    `);
        ...
        try {
            await pool.query(`ALTER TABLE opportunities MODIFY COLUMN type VARCHAR(100) NOT NULL`);
        }
        catch (_e) { }
        try {
            await pool.query(`ALTER TABLE opportunities ADD COLUMN target_audience VARCHAR(50) NOT NULL DEFAULT 'STUDENT'`);
        }
        catch (_e) { }
    ...
    `);
        ...
        const [existingInstUser] = await pool.query(`SELECT COUNT(*) as count FROM users WHERE LOWER(role) = 'institution'`);
        if (existingInstUser[0].count === 0) {
            const bcrypt = await import("bcrypt");
            const hashedInstPassword = await bcrypt.default.hash("InstitutionPassword123!", 10);
            await pool.query(`INSERT INTO users (name, username, email, password, role, institution_id)
         VALUES ('JIS University Academic Office', 'jis_institution', 'institution@jisuniversity.ac.in', ?, 'institution', 1)`, [hashedInstPassword]);
            console.log("Default institution account seeded: institution@jisuniversity.ac.in");
        }
        // Set default institution_id for unassigned student profiles
        await pool.query(`UPDATE student_profiles SET institution_id = 1 WHERE institution_id IS NULL`);
       ...
       WHERE name LIKE '%MySQL%' OR name LIKE '%SQL%' OR name LIKE '%Relational%'`);
        ...
        const [existingCollabs] = await pool.query(`SELECT COUNT(*) as count FROM collaborations`);
        if (existingCollabs[0].count === 0) {
            const [adminUser] = await pool.query(`SELECT id FROM users WHERE LOWER(role) = 'admin' LIMIT 1`);
           ...
           VALUES (?, 1, ?, ?, ?, ?, ?, ?, ?, 'published')`, [
                ...
                for (const sName of col.skills) {
                    ...
                    if (sRows.length > 0) {
                        await pool.query(`INSERT INTO collaboration_skills (collaboration_id, skill_id) VALUES (?, ?)`, [newCollabId, sRows[0].id]);
                    }
                }
            }
            console.log("Seeded sample Academia-Industry collaboration initiatives.");
        }
        console.log("Database schema reset, skills, assessment questions, and collaborations populated successfully.");
    }
    catch (error) {
