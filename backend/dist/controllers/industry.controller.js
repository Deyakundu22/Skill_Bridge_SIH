...
const formatIndustryProfile = (row) => ({
    id: row.id,
    userId: row.user_id,
...
export const getProfile = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(`SELECT * FROM industry_profiles WHERE user_id = ?`, [userId]);
        if (!rows || rows.length === 0) {
            ...
            await pool.query(`INSERT INTO industry_profiles (user_id, company_name, contact_email, verification_status)
         VALUES (?, ?, ?, 'pending')
         ON DUPLICATE KEY UPDATE user_id=user_id`, [userId, defaultCompanyName, defaultEmail]);
            const [reQueried] = await pool.query(`SELECT * FROM industry_profiles WHERE user_id = ?`, [userId]);
...
export const createProfile = async (req, res) => {
    ...
    try {
        ...
        const [existing] = await pool.query(`SELECT id FROM industry_profiles WHERE user_id = ?`, [userId]);
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')`, [
        ...
        const [createdRows] = await pool.query(`SELECT * FROM industry_profiles WHERE user_id = ?`, [userId]);
        res.status(201).json({
            success: true,
...
export const updateProfile = async (req, res) => {
    ...
    try {
      ...
      ON DUPLICATE KEY UPDATE
        ...
        const [updatedRows] = await pool.query(`SELECT * FROM industry_profiles WHERE user_id = ?`, [userId]);
        res.status(200).json({
            success: true,
