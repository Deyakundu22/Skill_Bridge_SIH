...
export const getStudentProfile = async (req, res) => {
    ...
    try {
       ...
       ORDER BY ss.id DESC`, [studentProfileId]);
        // 3. Fetch Projects
        const [projectRows] = await pool.query(`SELECT id, title, description, tech_stack, status, project_url, repo_url 
       FROM student_projects 
       WHERE student_id = ?
       ORDER BY id DESC`, [studentProfileId]);
        ...
        const [certifications] = await pool.query(`SELECT id, title, issuer, issue_year, credential_url 
       FROM student_certifications 
       WHERE student_id = ?
       ORDER BY id DESC`, [studentProfileId]);
        const primaryRole = Array.isArray(profile.target_roles) && profile.target_roles.length > 0
...
export const updateStudentProfile = async (req, res) => {
    ...
    const { name, phone, location, dob, gender, bio, institution_id, degree, department, roll_number, student_id, studentId, current_sem, cgpa, expected_grad, counselor, github, linkedin, portfolio, work_mode_preference, expected_stipend_min, expected_stipend_max, preferred_locations, target_roles, } = req.body;
    try {
...
export const getInstitutions = async (_req, res) => {
    try {
        const [rows] = await pool.query(`SELECT id, name, code, location, website, verification_status 
       FROM institutions 
...
const getStudentProfileId = async (userId) => {
    ...
    if (rows.length === 0) {
        const [result] = await pool.query(`INSERT INTO student_profiles (user_id) VALUES (?)`, [userId]);
...
export const importGitHubProjects = async (req, res) => {
    ...
    try {
        ...
        const [updatedProjects] = await pool.query(`SELECT id, title, description, tech_stack, status, project_url, repo_url 
       FROM student_projects 
       WHERE student_id = ? 
 ...
 */
export const addStudentProject = async (req, res) => {
    const userId = req.user?.id;
    if (!userId) {
        res.status(401).json({ error: "Unauthorized" });
        return;
