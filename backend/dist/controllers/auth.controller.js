...
export const getCaptcha = async (_req, res) => {
    try {
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (!result.success) {
            res.status(400).json({
                success: false,
                message: result.error.issues[0]?.message || "Invalid password reset payload.",
            });
            return;
        }
        ...
        const tokenHash = crypto
            ...
            .digest("hex");
        await ensurePasswordResetTable();
        const [tokenRows] = await pool.query(`SELECT * FROM password_reset_tokens WHERE token_hash = ? AND used_at IS NULL AND expires_at > NOW() LIMIT 1`, [tokenHash]);
        ...
        const [userRows] = await pool.query(`SELECT id FROM users WHERE id = ? LIMIT 1`, [resetRecord.user_id]);
        ...
        await pool.query(`DELETE FROM password_reset_tokens WHERE user_id = ? AND id != ?`, [resetRecord.user_id, resetRecord.id]);
        res.status(200).json({
            success: true,
...
export const signUp = async (req, res) => {
    try {
        ...
        const { name, username, email, password, role, captchaId, captchaAnswer } = result.data;
        ...
        const inputInstitutionName = req.body.institution_name ||
